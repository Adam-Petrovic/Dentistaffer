...
export default function ResetRequest() {
  ...
  if (result) {
    return (
      <AuthLayout title="Token issued" subtitle="Use this token to reset your password.">
        <div className="alert alert-info">
          Token expires on {new Date(result.expiresAt).toLocaleString()}.
        </div>
        ...
        <button
          ...
          style={{ marginTop: 16 }}
          onClick={() => navigate(`/reset/complete?token=${result.resetToken}&email=${encodeURIComponent(email)}`)}
        >
          Reset Password
        </button>
      </AuthLayout>
